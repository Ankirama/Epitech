/*
** simple_list.c for list in /home/user/rendu/piscine_cpp_d02a/ex00
**
** Made by 
** Login   
**
** Started on  Thu Jan  8 15:53:42 2015 
** Last update Fri Jan  9 08:16:01 2015 
*/
...
t_bool	list_del_node(t_list *front_ptr, t_node *node_ptr)
{
  ...
  if (*front_ptr == NULL || node_ptr == NULL)
    return (FALSE);
  head = *front_ptr;
  if (head->next == NULL && head == node_ptr)
    {
      free(*front_ptr);
      *front_ptr = NULL;
      return (TRUE);
    }
  while (head != NULL && head->next != NULL && head->next != node_ptr)
    head = head->next;
  if (head == NULL)
    return (FALSE);
  tmp = head->next;
