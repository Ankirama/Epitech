...
** Last update Sat Jan 10 03:27:37 2015 Fabien Martinez
*/
...
String*		substr(String *this, int offset, int length)
{
  ...
  i = offset >= 0 ? offset : (int)strlen(this->str) - 1;
  if ((i >= 0 && i > (int)strlen(this->str)) || (i < 0 && i * -1 >= (int)strlen(this->str)))
    return (NULL);
  ...
  if (length >= 0)
	...
	return (NULL);
      ...
      j = -1;
      while (this->str[i] != '\0' && ++j < length)
	{
	  new->str[j] = this->str[i];
	  ++i;
	}
      new->str[++j] = '\0';
    }
    ...
    {
      new->str = malloc(sizeof(char) * (length * -1 + 1));
      j = -1;
      while (i >= 0 && length < 0)
	{
	  new->str[++j] = this->str[i];
