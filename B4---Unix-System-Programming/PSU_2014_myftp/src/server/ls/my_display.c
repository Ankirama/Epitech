/*
** my_display.c for my_display in /home/user/rendu/PSU_2013_my_ls
**
** Made by 
** Login   
**
** Started on  Wed Apr 23 20:32:01 2014 
** Last update Fri May  2 18:54:40 2014 
*/
...
void		my_putfile(char file, t_my_file *files, char *opt, char end)
{
  ...
  while (files != NULL)
    {
      if (!(opt[BACKUP] && files->name[strlen(files->name) - 1] == '~') &&
...
{
  errno = my_errno;
  my_printf(g_fd, "my_ls: cannot open %s: %m\n", str);
  return (EXIT_FAILURE);
}
