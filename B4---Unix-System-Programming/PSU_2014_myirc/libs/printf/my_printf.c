/*
** my_printf.c for my_printf in /home/user/rendu/PSU_2014_my_printf
**
** Made by 
** Login   
**
** Started on  Tue Apr  8 13:48:42 2014 
** Last update Sun Apr 20 22:52:47 2014 
*/
			...
			const char *str, int *i)
{
  ...
  *i += 1;
  ...
  if ((flag = my_check_lh(str, s, *i, 'l')) >= 0)
    ...
    {
      flag_opt->count += my_write(g_fd, &(str[*i - 1]), 1);
      flag_opt->count += my_write(g_fd, &(str[*i]), 1);
      *i += 1;
    }
}
...
int		my_printf(int fd, const char *format, ...)
{
  ...
  while (format != NULL && format[i] != 0)
	...
	flag_opt->count += my_write(g_fd, &(format[i]), 1);
      i += 1;
    }
