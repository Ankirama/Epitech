/*
** get_next_line.c for get_next_line in /home/user/rendu/CPE_2013_getnextline
**
** Made by 
** Login   
**
** Started on  Tue Apr 15 11:53:13 2014 
** Last update Fri Apr 25 20:01:45 2014 
*/
...
/*
** This is a special _my_len because it will count until a char OR \0 (ofc)
*/
...
static char	_my_rea(char **base, char *src, char my_glob)
{
  ...
  if ((res = malloc(_my_len((*base), 0) + _my_len(src, '\n') + 1)) == NULL)
    return (-1);
  i = 0;
  while ((*base)[i])
    {
  ...
  free((*base));
  (*base) = res;
  if (j != _my_len(src, '\0') && my_add(&src[_my_len(src, '\n')], my_glob))
    return (-1);
...
** In fact, it'll use _my_rea to create of str until \n OR the EOF
*/
