        ...
        if (Admin.isAuthorized(session)) {
            if (request.getPathInfo() != null && request.getPathInfo().length() > 1) {
                try {
                    ...
                    if ((boolean) DBManager.executeQuery(new ModelProduct(), "enableProductById", id)) {
                        request.setAttribute("message", "Your product has been enabled.");
                    } else {
                        request.setAttribute("message", "Unable to enable this product.");
                    }
